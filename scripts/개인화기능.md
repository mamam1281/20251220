...
참고로 AdminUser 응답의 season_level은 user 테이블 컬럼이 아니라, 시즌패스 진행도(SeasonPassProgress.current_level)를 active 시즌 기준으로 합성(enrich)해서 내려주는 값이다. 동일하게 xp도 admin 사용자 목록/상세에서는 active 시즌의 SeasonPassProgress.current_xp를 합성해 내려줄 수 있다.
