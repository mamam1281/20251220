...
def downgrade() -> None:
    ...
    op.execute(
        """
        DELETE FROM season_pass_level WHERE season_id = 1;
        DELETE FROM season_pass_config WHERE id = 1;
        DELETE FROM feature_schedule WHERE date = CURDATE();
        DELETE FROM feature_config WHERE feature_type = 'ROULETTE';
