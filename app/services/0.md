...
로그인 → 토큰 발급 → 홈에서 “내 레벨 + 보유 코인 + 게임 리스트” 한 번에 보여주기 → 게임 선택 → 플레이 → DB에 로그/레벨 반영 → 관리자가 로그 보고 다른 시스템에서 보상.
...
- frontend에서 Admin 관련 라우터/페이지가 있는지 전체 검색:
  ...
  - 없으면:
    ...
    - 최소한 "user_id 입력 → 해당 유저 최근 로그/레벨 상태를 보여주는 화면" 정도는 scaffold 형태로 생성해줘.

7. 한 문장으로 다시 정리하면

지금은 “크리스마스 1주일 / today-feature / TEST_MODE” 다 집어넣느라 꼬여 있으니까,
일단 로그인 → 홈 → 여러 게임 + 레벨가 항상 보이고,
